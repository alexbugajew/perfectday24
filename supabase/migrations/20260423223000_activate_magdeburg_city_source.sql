...
select
  ...
  'magdeburg',
  'DE',
  'https://www.magdeburg.de/veranstaltungen',
  'html',
  'Magdeburg',
  'Offizieller Veranstaltungsexport der Landeshauptstadt Magdeburg ueber RSS plus konkrete Event-Detailseiten aus dem offiziellen Stadtkalender.',
  83,
  true
where not exists (
  select 1
  from public.event_source_configs
  where provider = 'magdeburg_city'
    and city_slug = 'magdeburg'
);

update public.event_source_configs
set
  country_code = 'DE',
  base_url = 'https://www.magdeburg.de/veranstaltungen',
  parser_mode = 'html',
  label = 'Magdeburg',
  notes = 'Offizieller Veranstaltungsexport der Landeshauptstadt Magdeburg ueber RSS plus konkrete Event-Detailseiten aus dem offiziellen Stadtkalender.',
  priority = 83,
  is_active = true,
  updated_at = now()
where provider = 'magdeburg_city'
