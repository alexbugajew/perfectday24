alter table public.event_source_configs
  drop constraint if exists event_source_configs_provider_check;

alter table public.event_source_configs
  add constraint event_source_configs_provider_check
  check (
    provider = any (
      array[
        'visitberlin'::text,
        'berlin_de'::text,
        'hamburg_tourism'::text,
        'hamburg_de'::text,
        'hamburg_infomax'::text,
        'muenchen_de'::text,
        'koeln_tourism'::text,
        'frankfurt_tourism'::text,
        'stuttgart_tourism'::text,
        'duesseldorf_tourism'::text,
        'leipzig_travel'::text,
        'dresden_tourism'::text,
        'hannover_tourism'::text,
        'nuernberg_tourism'::text,
        'bremen_tourism'::text,
        'dortmund_tourism'::text,
        'mannheim_tourism'::text,
        'wiesbaden_tourism'::text,
        'bonn_city'::text,
        'visit_essen'::text,
        'karlsruhe_tourism'::text,
        'muenster_tourism'::text,
        'aachen_city'::text,
        'augsburg_city'::text,
        'kiel_sailing_city'::text,
        'bielefeld_jetzt'::text,
        'braunschweig_region'::text,
        'bochum_tourism'::text,
        'duisburg_live'::text,
        'wuppertal_live'::text,
        'freiburg_eventportal'::text
      ]
    )
  );

insert into public.event_source_configs (
  provider,
  city_slug,
  country_code,
  base_url,
  parser_mode,
  label,
  notes,
  priority,
  is_active
)
select
  'freiburg_eventportal',
  'freiburg-im-breisgau',
  'DE',
  'https://veranstaltungen.freiburg.de/freiburg/events',
  'api',
  'Freiburg Eventportal',
  'Offizielles Freiburg-Eventportal ueber den FWTM/imxplatform-GraphQL-Pfad mit paginierten Event-Knoten, Venue-/Geo-Daten und Occurrences aus eventDates.',
  80,
  true
where not exists (
  select 1
  from public.event_source_configs
  where provider = 'freiburg_eventportal'
    and city_slug = 'freiburg-im-breisgau'
);

update public.event_source_configs
set
  country_code = 'DE',
  base_url = 'https://veranstaltungen.freiburg.de/freiburg/events',
  parser_mode = 'api',
  label = 'Freiburg Eventportal',
  notes = 'Offizielles Freiburg-Eventportal ueber den FWTM/imxplatform-GraphQL-Pfad mit paginierten Event-Knoten, Venue-/Geo-Daten und Occurrences aus eventDates.',
  priority = 80,
  is_active = true,
  updated_at = now()
where provider = 'freiburg_eventportal'
  and city_slug = 'freiburg-im-breisgau';
