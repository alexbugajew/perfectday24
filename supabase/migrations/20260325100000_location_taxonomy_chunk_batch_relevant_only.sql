...
create or replace function public.pd24_backfill_location_taxonomy_city_chunk(
  p_city_slug text,
...
begin
  with candidate_rows as (
    ...
    where city_slug = p_city_slug
      ...
      and (
        category::text in ('restaurant', 'cafe', 'activity', 'culture', 'nightlife', 'event')
        or type in (
          'museum',
          'gallery',
          'arts_centre',
      ...
      and (
        ...
        or coalesce(cardinality(audiences), 0) = 0
        or coalesce(cardinality(occasions), 0) = 0
        or coalesce(enrichment_version, 0) < 2
      )
    order by
      case
        when category::text = 'culture' then 1
        when category::text = 'activity' then 2
        when category::text = 'nightlife' then 3
        when category::text = 'restaurant' then 4
        when category::text = 'cafe' then 5
        else 6
      end,
      id
  ...
  enriched as (
    select
      ...
      public.pd24_unique_text_array(
        ...
        case when type in ('place_of_worship') then array['historic_site', 'landmark'] else '{}'::text[] end ||
        ...
        case when category::text = 'event' then array['event_social'] else '{}'::text[] end ||
