...
select
  ...
  'luebeck',
  'DE',
  'https://www.luebeck-tourismus.de/veranstaltungen',
  'api',
  'Luebeck Tourismus',
  'Offizieller Luebeck-Tourismuskalender ueber den Datacycle-Endpoint der Veranstaltungsseite plus Event-Detailseiten mit vollstaendigem JSON-LD fuer Venue, Geo und eventSchedule-Occurrences.',
  81,
  true
where not exists (
  select 1
  from public.event_source_configs
  where provider = 'luebeck_tourism'
    and city_slug = 'luebeck'
);

update public.event_source_configs
set
  country_code = 'DE',
  base_url = 'https://www.luebeck-tourismus.de/veranstaltungen',
  parser_mode = 'api',
  label = 'Luebeck Tourismus',
  notes = 'Offizieller Luebeck-Tourismuskalender ueber den Datacycle-Endpoint der Veranstaltungsseite plus Event-Detailseiten mit vollstaendigem JSON-LD fuer Venue, Geo und eventSchedule-Occurrences.',
  priority = 81,
  is_active = true,
  updated_at = now()
where provider = 'luebeck_tourism'
