...
export async function PATCH(
  ...
  if (body.is_active && !["approved", "published"].includes(reviewStatus)) {
  ...
  if (updateErr) {
    console.error("affiliate link patch error:", updateErr);
...
export async function DELETE(
  ...
  const { error: deleteErr } = await admin.from("affiliate_links").delete().eq("id", id);
  if (deleteErr) {
    console.error("affiliate link delete error:", deleteErr);
