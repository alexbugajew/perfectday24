...
function estimateDurationMin(loc: LocationRow) {
  if (typeof loc.duration_min === "number" && Number.isFinite(loc.duration_min) && loc.duration_min > 0) {
...
function interestWeights(owner: string[], members: GroupMember[], enabled: boolean) {
  ...
  const addList = (arr: string[]) => {
    for (const x of arr.map(norm).filter(Boolean)) counts.set(x, (counts.get(x) ?? 0) + 1);
  };

  addList(owner);
  if (enabled) for (const m of members) addList(m.interests ?? []);
...
function slotCategoryMatch(kind: SlotKind, loc: LocationRow) {
  ...
  const meal = resolveMeal(loc); // manual_meal > meal
...
export default function Home() {
  ...
  const [originMode, setOriginMode] = useState<"auto" | "city">("auto"); // auto = GPS, city = center/fallback
  ...
  // Fullday: Anzahl Aktivitäten zwischen den Mahlzeiten
  ...
  // StopsCount begrenzen je Mode (außer fullday)
  useEffect(() => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await supabase
        .from("cities")
        .select("slug,name,center_lat,center_lng,population")
        .eq("is_active", true)
      ...
      // Default City: erste aktive City (höchste population)
      if (!selectedCitySlug && list.length) {
        setSelectedCitySlug(list[0].slug);
        setCityCenterLat(list[0].center_lat ?? null);
        setCityCenterLng(list[0].center_lng ?? null);
      }
  ...
  // ✅ Bei City-Wechsel Center setzen
  useEffect(() => {
    if (!selectedCitySlug) return;
    const c = cities.find((x) => x.slug === selectedCitySlug);
    setCityCenterLat(c?.center_lat ?? null);
    setCityCenterLng(c?.center_lng ?? null);
  ...
  // ✅ Locations laden (scoped by city_slug)
  useEffect(() => {
    if (!selectedCitySlug) return;
    ...
    (async () => {
      ...
      // ✅ Fallback City Center: wenn cities.center_lat/lng NULL ist,
      // nutze den Mittelwert der vorhandenen Location-Koordinaten.
      const hasCityCenter = typeof cityCenterLat === "number" && typeof cityCenterLng === "number";
      if (!hasCityCenter) {
        const coords = list.filter((l) => typeof l.lat === "number" && typeof l.lng === "number") as Array<
          LocationRow & { lat: number; lng: number }
        >;
        if (coords.length) {
          const avgLat = coords.reduce((s, x) => s + x.lat, 0) / coords.length;
          const avgLng = coords.reduce((s, x) => s + x.lng, 0) / coords.length;
          setCityCenterLat(Math.round(avgLat * 1e6) / 1e6);
          setCityCenterLng(Math.round(avgLng * 1e6) / 1e6);
        }
      }
  ...
  async function saveProfileInterests(next: string[]) {
    ...
    try {
      ...
      const { error } = await supabase.from("profiles").upsert({ user_id: userId, interests: uniq }, { onConflict: "user_id" });
  ...
  // ✅ Origin bestimmen: GPS oder City Center
  const originLat = originMode === "auto" && userLat != null ? userLat : cityCenterLat;
  const originLng = originMode === "auto" && userLng != null ? userLng : cityCenterLng;
  ...
  const { results, activeLevel, effectiveRadiusKm } = useMemo(() => {
    ...
    const withDistance: (LocationRow & { distanceKm: number | null })[] = locations.map((loc) => {
      ...
      if (originLat != null && originLng != null && typeof loc.lat === "number" && typeof loc.lng === "number") {
    ...
    const radiusSteps =
      originLat != null && originLng != null ? uniqueNumbers([radiusKm, 20, 35, 50]).sort((a, b) => a - b) : [radiusKm];
  ...
  const plannedStops: PlannedStop[] = useMemo(() => {
    ...
    const getPoolForKind = (kind: SlotKind, mode: PlanMode) => {
      ...
      // Breakfast: prefer cafe, then foodish, last resort: anything
      if (kind === "breakfast") {
      ...
      // Lunch/Dinner: prefer restaurants, then cafes, then foodish
      ...
      // Activity: actish, then other, then non-food
      if (kind === "activity") {
      ...
      // Anything: in evening prefer nightlife
      if (kind === "anything") {
        if (mode === "evening") {
    ...
    for (let i = 0; i < slotCount; i++) {
      ...
      const scored = pool
        .map((cand) => {
          ...
          // Slot Boost (MVP)
          const slotBoost =
            isMealSlot ? (strictMatch ? 35 : 10) : slot.kind === "activity" ? (strictMatch ? 18 : 6) : 0;

          const total = cand.totalScore - travelPenalty + slotBoost;
      ...
      const res = await fetch("/api/generate-plan-text", {
        ...
        body: JSON.stringify({
          filters: {
            ...
            // ✅ expose city context (optional for AI)
            selectedCitySlug,
            originMode,
          },
          radiusKm,
  ...
  async function sharePlan(plan: SavedPlanRow) {
    ...
    if (!token) {
      ...
      const { error } = await supabase.from("plans").update({ share_token: token }).eq("id", plan.id).eq("user_id", userId);
  ...
  const expandedText =
    originLat != null && originLng != null && effectiveRadiusKm != null && effectiveRadiusKm > radiusKm
      ? `Um mehr Optionen zu finden, haben wir den Umkreis intern auf ${effectiveRadiusKm} km erweitert.`
      : null;

  const quickTags = ["sushi", "italien", "vegan", "steak", "theater", "konzerte", "museum", "sport", "natur", "stadt", "techno", "jazz"];
  ...
  // UI helper
  const selectedCity = selectedCitySlug ? cities.find((c) => c.slug === selectedCitySlug) : null;

  return (
    <main className="p-10 max-w-4xl mx-auto">
      <h1 className="text-4xl font-bold mb-2">PerfectDay24 🚀</h1>
      <p className="text-gray-600 mb-6">Smart Local Planner – mit Vorlieben, Gruppe, Umkreis, Dauer & Wegezeit.</p>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-xl p-5">
            <div className="flex items-start justify-between gap-3 mb-2">
              <div>
                ...
                <div className="text-sm text-gray-600">Wähle ein paar Interessen (max. 12). Das beeinflusst alle Vorschläge.</div>
              </div>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setShowPrefsModal(false)} className="px-4 py-2 rounded bg-black text-white text-sm">
                Fertig ✅
              </button>
            </div>
          </div>
      ...
      <div className="p-4 border rounded-lg mb-6 space-y-4">
        <div className="flex gap-3 flex-wrap">
          ...
          >
            {cities.length === 0 ? <option value="">Keine Cities</option> : null}
            {cities.map((c) => (
          ...
          {/* ✅ Origin Mode */}
          <select value={originMode} onChange={(e) => setOriginMode(e.target.value as any)} className="border p-2 rounded">
          ...
          <select value={planMode} onChange={(e) => setPlanMode(e.target.value as PlanMode)} className="border p-2 rounded">
          ...
          <div className="p-3 border rounded-lg flex flex-col gap-3">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setFullDayActsAfterBreakfast((v) => clamp(v - 1, 1, 2))} className="px-3 py-2 rounded border text-sm">
                  –
                </button>
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setFullDayActsAfterLunch((v) => clamp(v - 1, 1, 2))} className="px-3 py-2 rounded border text-sm">
                  –
                </button>
          ...
          <div className="text-sm text-gray-600">
            {originMode === "auto" ? (
              userLat && userLng ? (
                ...
                <>Standort aus: {geoError} (Fallback: City Center)</>
              ) : (
                <>Standort wird geladen…</>
              )
            ) : (
              <>
                City Center aktiv ✅{selectedCity ? ` (${selectedCity.name})` : ""}{" "}
                {originLat == null || originLng == null ? "• (Center fehlt – Fallback über Locations)" : ""}
              </>
            )}
          </div>
        </div>
        ...
        <div className="text-xs text-gray-600">
          City: <span className="font-semibold">{selectedCity?.name ?? "—"}</span> • Zeitbudget: ~{timeBudgetForMode(planMode)} Min • Vorlieben:{" "}
          ...
          {groupEnabled ? (
            <>
              <div className="grid gap-2 md:grid-cols-3">
                <input value={memberName} onChange={(e) => setMemberName(e.target.value)} placeholder="Name (optional)" className="border p-2 rounded" />
                <input
              ...
              <div className="flex gap-2 flex-wrap">
                <button
                  onClick={() => {
                    ...
                    setGroupMembers((prev) => [...prev, { id: String(id), name: name || `Gast ${prev.length + 1}`, interests: list }]);
                    setMemberName("");
                ...
                <div className="space-y-2">
                  {groupMembers.map((m) => (
                    <div key={m.id} className="p-3 border rounded flex items-start justify-between gap-3">
                      ...
                      <button onClick={() => setGroupMembers((prev) => prev.filter((x) => x.id !== m.id))} className="px-3 py-2 rounded border text-sm">
                        Entfernen
                      </button>
                    </div>
                  ))}
              ...
              <div className="text-xs text-gray-600">Effektive Interessen: {effectiveInterests.length ? effectiveInterests.join(", ") : "—"}</div>
            </>
          ) : (
      ...
      <div className="p-4 border rounded-lg mb-6 space-y-3">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div>
            ...
            <div className="text-xs text-gray-600">Echte Wege über OSRM (ohne API-Key). Profil: {routeProfile === "foot" ? "zu Fuß" : "Auto"}</div>
          </div>

          <div className="flex items-center gap-2">
            <select value={routeProfile} onChange={(e) => setRouteProfile(e.target.value as any)} className="border p-2 rounded text-sm">
        ...
        <div className="p-3 border rounded-lg text-sm text-gray-700">
          ...
          {mapStops.length < 2 ? (
            ...
            <div className="text-xs text-gray-600">
              ...
              <div className="mt-2">
                Schätzung: ~{fallbackSummary.distanceKm} km • Aktivitäten ~{fallbackSummary.durationMin} Min • Wege ~{fallbackSummary.travelMin} Min • Gesamt ~
      ...
      <div className="p-4 border rounded-lg mb-6 space-y-4">
        <div className="flex gap-3 flex-wrap items-center">
          <input value={planTitle} onChange={(e) => setPlanTitle(e.target.value)} placeholder="Optionaler Titel (z.B. Date in Berlin)" className="border p-2 rounded flex-1 min-w-[240px]" />

          <button onClick={generateAIText} disabled={aiLoading} className="px-4 py-2 rounded border text-sm disabled:opacity-60">
        ...
        {aiText ? <div className="p-4 border rounded-lg text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{aiText}</div> : null}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-3 flex-wrap">
                      <h3 className="font-bold text-lg">
                        {stop.label} <span className="text-xs font-normal text-gray-500">• {stop.hint}</span>
                      </h3>
                    </div>
                  ...
                  <div className="flex flex-col gap-2 items-end">
                    ...
                    {stop.item?.reservation_url ? (
                      <a href={stop.item.reservation_url} target="_blank" rel="noreferrer" className="px-3 py-2 rounded border text-sm">
                        Reservieren
                      </a>
                    ) : null}
                  </div>
                ...
                <div key={p.id} className="p-4 border rounded-lg hover:bg-gray-50 flex items-start justify-between gap-4">
                  <button onClick={() => setSelectedPlan(p)} className="text-left flex-1">
                    ...
                    <div className="text-xs text-gray-500">
                      {new Date(p.created_at).toLocaleString()} • Mode: {p.filters?.planMode ?? "—"} • Stops: {p.filters?.stopsCount ?? "—"}
                    </div>
                    <div className="mt-2 inline-block text-xs px-2 py-1 rounded border">{p.active_level || "n/a"}</div>

                    {p.ai_description ? <div className="mt-2 text-xs text-gray-600 line-clamp-2">{p.ai_description}</div> : null}
                  </button>
            ...
            <div className="p-4 border rounded-lg">
              <div className="flex items-center justify-between gap-3 mb-3">
                <div>
                  ...
                  <div className="text-xs text-gray-500">
                    Radius: {selectedPlan.radius_km} km • Sort: {selectedPlan.sort_mode} • Mode: {selectedPlan.filters?.planMode ?? "—"} • Stops:{" "}
                    {selectedPlan.filters?.stopsCount ?? "—"}
                  </div>
                </div>
              ...
              {selectedPlan.ai_description ? (
                <div className="p-3 border rounded-lg text-sm text-gray-700 whitespace-pre-wrap mb-3">{selectedPlan.ai_description}</div>
              ) : null}
      ...
      {toast ? (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-black text-white text-sm px-4 py-2 rounded-lg shadow-lg z-50">{toast}</div>
      ) : null}
    </main>
  );
}