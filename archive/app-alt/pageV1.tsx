...
function normalizeDaytime(v: string | null | undefined): "morning" | "midday" | "evening" | "night" | null {
  ...
  if (x === "afternoon") return "evening"; // <- wichtig: altes System
...
function classify(loc: LocationRow) {
  // ✅ Priorität: manual_category > DB category > Heuristik
  ...
  if (
    ...
    return "activity";
  if (has("event", "konzert", "concert", "festival", "show", "ticket"))
    return "event";
...
function resolveMeal(loc: LocationRow): MealType {
  return ((loc.manual_meal ?? loc.meal) ?? null) as MealType;
}

function slotCategoryMatch(kind: SlotKind, loc: LocationRow) {
  const cat = classify(loc); // nutzt DB category wenn vorhanden, sonst Heuristik
  const meal = (loc.meal ?? null) as MealType;

  // Meals: wenn meal gepflegt ist, ist das die stärkste Quelle.
  // Wenn meal NULL ist, fallbacken wir über category.
  if (kind === "breakfast") {
  ...
  // Aktivitäten: nicht Essen, sondern Erlebnis-Kategorien
  if (kind === "activity") {
    return cat === "culture" || cat === "activity" || cat === "event";
  }

  // Anything: alles erlaubt
  return true;
}
...
const getPoolForKind = (kind: SlotKind, mode: PlanMode) => {
  ...
  // Ziel: Plan nie komplett leer, aber trotzdem “sinnvoll”.
  ...
  if (kind === "breakfast") {
    ...
    return candidates; // letzter Notnagel
  }
  ...
  if (kind === "activity") {
    ...
    // letzte Stufe: alles außer Essen
    ...
    for (let i = 0; i < slotCount; i++) {
      ...
      const scored = pool
        .map((cand) => {
          ...
          const penalty = travelMin != null ? Math.min(40, Math.round(travelMin / 3)) : 0;
          const isMealSlot = slot.kind === "breakfast" || slot.kind === "lunch" || slot.kind === "dinner";
	  const strictMatch = slotCategoryMatch(slot.kind, cand);

	  // MealSlots: stark belohnen, wenn wirklich Meal passt.
	  // Activity: leicht belohnen, wenn wirklich activity/culture/event.
	  const slotBoost = isMealSlot ? (strictMatch ? 35 : 10) : slot.kind === "activity" ? (strictMatch ? 18 : 6) : 0;

          const total = cand.totalScore - penalty + slotBoost;
  ...
  return (
    <main className="p-10 max-w-4xl mx-auto">
      ...
      <div className="p-4 border rounded-lg mb-6 space-y-4">
        ...
        <div className="text-xs text-gray-600">
          Zeitbudget: ~{timeBudgetForMode(planMode)} Min • Vorlieben:{" "}
